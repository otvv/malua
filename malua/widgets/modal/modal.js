...
class MModal extends HTMLElement {
  ...
  constructor() {
    ...
    <section class="m-modal-background">
    </section>
  ...
  `;
    ...
    const attributesToSet = ['title', 'placeholder', 'shader', 'type', 'width', 'height'];
    ...
    modalOpenButtonElement.textContent = this.getAttribute('placeholder') || this.getAttribute('title')
    ...
    if (this.hasAttribute('shader') || this.hasAttribute('effect')) {
      ...
      modalElement.classList.add(this.getAttribute('shader') || this.getAttribute('effect'))
      ...
      modalBackgroundElement.classList.add(this.getAttribute('shader') || this.getAttribute('effect'))
    }

    const modalElementsGroup = [ modalBackgroundElement, modalBoxElement, modalElement ]

    // handle modal open and close "operations"
    this.handleModalOperation(modalOpenButtonElement, modalElementsGroup, 'open', 1.0) /* (1.0/100) = 100% opacity */
    this.handleModalOperation(modalCloseButtonElement, modalElementsGroup, 'close', 0.0) /* (0.0/100) = 0% opacity */
